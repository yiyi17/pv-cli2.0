'use strict';
const utils = require('@pvjs/pv-utils');
...
async function checkGlobalUpdate() {
  ...
  // 获取所有版本
  const { getNpmSemverVersion } = require('@pvjs/pv-get-npm-info');
  try {
    ...
    if (lastVersion && semver.gt(lastVersion, currenVersion)) {
      log.warn(
        colors.yellow(`请手动更新 ${npmName}, 当前版本是: ${currenVersion}, 最新版本: ${lastVersion}
